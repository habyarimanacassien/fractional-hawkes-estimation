...
truealpha <- 0.5    # true value of the parameter to estimate 
lambda <- 0.7 
...
for (iT in 1:length(Timev)){  #iT in 1:2
  ...
  for (iter in 1:Max_iter){ # iter in 1:5
    ...
    ParameterSample_values <- runif(Niter,min=0.001,max=0.999)    # We generate a uniformly distributed prior
...
for (iter in 1:Max_iter){
  ...
  for (iT in 1:length(Timev)){
    ...
    dx <- 1/60 #bin size
    LL <- 1/60 # lower limit of the breaks interval

...
for (iT in 1:length(Timev)){  #iT in 1:2
  ...
  for (iter in 1:Max_iter){ # iter in 1:5
    ...
    SimPoints <- simulate_points(lambda=lambda,alpha=alpha,beta=truebeta,Time=Time)
    ...
    #Simulated data (SimPointsth) with uniform prior for BETA parameter
    cl <- makeCluster(number_of_cores)  # 'number_of_cores' is the number of clusters (cores)
    SimPointsth_list <- parSapply(cl , ParameterSample_values, simulate_points, lambda=lambda, alpha=alpha, Time = Time)
    stopCluster(cl)
